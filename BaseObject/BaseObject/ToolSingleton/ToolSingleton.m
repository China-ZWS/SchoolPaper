...
- (NSString * )getLocationDistanceWithLatitude:(NSString *)latitude longitude:(NSString *)longitude
{
    //第一个坐标
    CLLocation *current=[[CLLocation alloc] initWithLatitude:[self.latitudes integerValue] longitude:[self.longitudes integerValue]];
    //第二个坐标
    CLLocation *before=[[CLLocation alloc] initWithLatitude:[latitude integerValue] longitude:[longitude integerValue]];
    // 计算距离
    ...
    {
        return [NSString stringWithFormat:@"%.0f公里",meters / 1000.0];
    }
}
