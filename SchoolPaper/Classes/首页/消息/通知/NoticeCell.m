...
- (void)setDatas:(id)datas
{
    ...
    _datas = datas;
    NSString *name = datas[@"userName"];
    _name.text = name;
    [_name sizeToFit];
    
    
    NSString *weekDay = [NSObject getWeekDay:datas[@"informtime"] format:@"yyyy-MM-dd"];
    ...
    NSDate *yearDate=[dateFormatter dateFromString:datas[@"informtime"]];
    ...
    NSString *createTime = [datas[@"informtime"] componentsSeparatedByString:@" "][0];
    ...
    NSMutableAttributedString *content = [[NSMutableAttributedString alloc] initWithString:[[datas[@"subject"] stringByAppendingString:@"："]stringByAppendingString:datas[@"msg"]]];
    
    [content addAttribute:NSForegroundColorAttributeName value:CustomBlue range:NSMakeRange(0,content.length - [datas[@"msg"] length])];
    [content addAttribute:NSForegroundColorAttributeName value:CustomGray range:NSMakeRange([datas[@"subject"] length] + 1, [datas[@"msg"] length])];
    [content addAttribute:NSFontAttributeName value:Font(17) range:NSMakeRange(0, [datas[@"subject"] length])];
    
    CGSize contentSize = [NSObject getSizeWithText:[[datas[@"subject"] stringByAppendingString:@"："]stringByAppendingString:datas[@"msg"]] font:_title.font maxSize:CGSizeMake(DeviceW - (CGRectGetMaxX(_headImageView.frame) + ScaleW(15) + ScaleW(5)), MAXFLOAT)];
